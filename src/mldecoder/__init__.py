...
from .models import CnnDecoder, MlpDecoder, RandomForestDecoder, XGBoostDecoder
...
def register_ml_decoders(*, train_shots: int = 50_000, train_seed: int = 1234) -> None:
    ...
    Importing this package already registers ``rf``, ``xgb``, ``mlp`` and ``cnn``
    with default settings. Call this to trade training cost against accuracy,
    e.g. a small ``train_shots`` for quick experiments or CI.
    """
    register_decoder(
    ...
    register_decoder("cnn", lambda: CnnDecoder(train_shots=train_shots, train_seed=train_seed))
